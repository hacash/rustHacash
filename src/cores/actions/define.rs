// action kind define \ parse func create \ include!() 
actions_kind_define_parse_func_include!(
    ACTION_KIND_1  , 1  , Action1HacTransfer                          ,
    ACTION_KIND_2  , 2  , Action2OpenChannel                          ,
    ACTION_KIND_3  , 3  , Action3CloseChannel                         ,
    ACTION_KIND_4  , 4  , Action4DiamondCreate                        ,
    ACTION_KIND_5  , 5  , Action5DiamondTransfer                      ,
    ACTION_KIND_6  , 6  , Action6DiamondMultipleTransfer              ,
    ACTION_KIND_7  , 7  , Action7SatoshiGenesis                       ,
    ACTION_KIND_8  , 8  , Action8SatoshiTransfer                      ,
    ACTION_KIND_9  , 9  , Action9LockblsCreate                        ,
    ACTION_KIND_10 , 10 , Action10LockblsRelease                      ,
    ACTION_KIND_11 , 11 , Action11SatoshiFromToTransfer               ,
    ACTION_KIND_12 , 12 , Action12CloseChannelWithBill                ,
    ACTION_KIND_13 , 13 , Action13HacFromTransfer                     ,
    ACTION_KIND_14 , 14 , Action14HacFromToTransfer                   ,


    ACTION_KIND_28 , 28 , Action28SatoshiFromTransfer                 ,
    ACTION_KIND_29 , 29 , Action29SubmitTimeLimit                 ,
    
    
    
);
